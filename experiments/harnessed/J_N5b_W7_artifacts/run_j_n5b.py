...
ARM_TO_CHANNEL = {"A0": "off", "A2": "verified"}


class ArtifactW7Model(jn3.W7HeldoutModel, artifacts.ArtifactBoundaryModel):
    """J-N3 W7 imported unchanged, with W8 artifact policy layered by MRO."""
...
def analyze(summary: dict[str, dict], thresholds: dict, seed_report: dict) -> dict:
    h = thresholds["H_C4_specificity_revenge"]
    a2 = summary["A2"]
    h4 = a2["false_containment"] <= h["A2_false_containment_max"] and a2["permanence_probability"] >= h["A2_permanence_min"]
    delta_false = summary["A0"]["false_containment"] - a2["false_containment"]
    return {
        "decision": "PASS" if seed_report["admissible"] and h4 else "FAIL",
        "decision_vector": {"H_C4_specificity_revenge": "PASS" if h4 else "FAIL"},
        "false_containment_delta_A0_minus_A2": delta_false,
        "A0_false_containment": summary["A0"]["false_containment"],
        "A2_false_containment": a2["false_containment"],
        "A2_permanence": a2["permanence_probability"],
    }
...
def tautology_report_from_a0(baseline_rows: list[dict]) -> tuple[dict, dict]:
    ...
    return {
        ...
        "baseline": "Fresh W7 A0 baseline rows over locked J-N5b seeds.",
        "baseline_summary": baseline_summary,
        "known_caveat": "A0 is the specificity-failure baseline from imported W7, not evidence for artifact benefit by itself.",
    }, baseline_summary
...
def experiment(precomputed_a0_rows: list[dict]) -> dict:
    ...
    rows.extend(run_rows(seeds, ["A2"]))
    summary = summarize(rows)
    seed_report = SP.enforce_seed_policy([
        {"metric": "J_N5b_W7_artifacts", "role": "core", "seeds": len(seeds), "pass_fail": "PASS"},
    ])
    analysis = analyze(summary, thresholds, seed_report)
    write_json(OUTPUTS / "j_n5b_checks.json", {"raw_runs": rows, "summary": summary, "analysis": analysis})
    return {
        "question": prereg["metadata"]["question"],
        "mode": prereg["metadata"]["mode"],
        "metric": "W7 A0 vs A2 false containment and permanence under imported J-N3 W7.",
        ...
        "downstream_consequence": "H-C4 combines with J-N5 H-C1 for the W8 kill-condition.",
        ...
        "inference": "Any false-containment reduction is attributed only through the preregistered artifact conformance mechanism.",
