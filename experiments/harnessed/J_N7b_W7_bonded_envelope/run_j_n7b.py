...
ARM_TO_CHANNEL = {"A0": "off", "P1-low": "verified", "P1-high": "verified"}
TRAJECTORY_FIELDS = ["declared_mass_by_step", "stiffness_by_step", "binding_events_by_step", "stake_balance_by_step", "stake_events_by_step"]
...
class BondedW7Model(jn3.W7HeldoutModel, artifacts.ArtifactBoundaryModel):
    """J-N3 W7 imported unchanged, with bonded artifact policy layered by MRO."""
...
def run_one(seed: int, arm: str) -> dict:
    ...
    model = BondedW7Model(seed, params)
    out = model.run()
    row = {
        "seed": seed,
        "world": jn3.W7_WORLD,
...
def analyze(summary: dict[str, dict], thresholds: dict, seed_report: dict) -> dict:
    h = thresholds["H_P3_W7_specificity"]
    a0_false = summary["A0"]["false_containment"]
    by_arm = {}
    for arm in ["P1-low", "P1-high"]:
        row = summary[arm]
        pass_bar = row["false_containment"] <= h["false_containment_max"] and row["permanence_probability"] >= h["permanence_min"]
        directional = a0_false - row["false_containment"]
        by_arm[arm] = {
            "verdict": "PASS" if pass_bar else "FAIL",
            "false_containment": row["false_containment"],
            "permanence": row["permanence_probability"],
            "directional_delta_A0_minus_arm": directional,
            "directional_delta_ge_0_10": directional >= h["recorded_directional_delta_min"],
        }
    h3_pass = any(v["verdict"] == "PASS" for v in by_arm.values())
    decision = "PASS" if seed_report["admissible"] and h3_pass else "FAIL"
    return {
        "decision": decision,
        "decision_vector": {"H_P3_W7_specificity": "PASS" if h3_pass else "FAIL", "by_arm": by_arm},
        "A0_false_containment": a0_false,
        "by_arm": by_arm,
    }
...
def tautology_report_from_a0(baseline_rows: list[dict]) -> tuple[dict, dict]:
    ...
    return {
        ...
        "baseline": "Fresh W7 A0 baseline rows over locked J-N7b seeds.",
        "baseline_summary": baseline_summary,
        "known_caveat": "A0 is the imported W7 baseline, not evidence for bonded artifact benefit by itself.",
    }, baseline_summary
...
def experiment(precomputed_a0_rows: list[dict]) -> dict:
    ...
    rows.extend(run_rows(seeds, ["P1-low", "P1-high"]))
    summary = summarize(rows)
    seed_report = SP.enforce_seed_policy([
        {"metric": "J_N7b_W7_bonded_envelope", "role": "core", "seeds": len(seeds), "pass_fail": "PASS"},
    ])
    analysis = analyze(summary, thresholds, seed_report)
    write_json(OUTPUTS / "j_n7b_checks.json", {"raw_runs": compact_rows(rows), "summary": summary, "analysis": analysis})
    ...
    return {
        ...
        "metric": "W7 A0 vs bonded P1 false containment and permanence under imported J-N3 W7.",
        ...
        "downstream_consequence": "H-P3 directional combines with J-N7 H-P2 L1 for the FW-3 kill condition.",
        ...
        "inference": "Any specificity improvement is attributed only through the preregistered bonded conformance mechanism.",
