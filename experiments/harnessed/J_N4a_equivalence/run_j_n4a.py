...
def run_headline_rows(thresholds: dict) -> list[dict]:
    ...
    for policy15 in thresholds["validation_policy15"]:
        params = families.params_for("validation", policy15, "", scenario="validation")
        for seed in seeds:
            rows.append(atlas.run_one(seed, params, "validation", 0.0, policy15, "validation"))
    for world in thresholds["worlds"]:
        for policy15 in thresholds["boundary_policy15"]:
            ...
            for seed in seeds:
                rows.append(atlas.run_one(seed, params, "adversarial_pressure", 1.0, "adversarial_pressure=1.00", "boundary"))
...
def experiment() -> dict:
    ...
    seed_report = SP.enforce_seed_policy([
        {"metric": "J_N4a_engineering_equivalence", "role": "core", "seeds": len(thresholds["seeds"]), "pass_fail": "PASS"},
    ])
    ...
    return {
        "question": "Do default-neutral J-N4 substrate extensions exactly preserve committed J-G1 headline summary cells?",
        ...
        "downstream_consequence": "PASS permits J-N4 preregistration; FAIL stops J-N4 until the implementation is fixed and re-reviewed.",
        "fact": "The model is run with neutral defaults t_act=0, v_prop=1.0, r_rec=1.0.",
        "inference": "Exact equality supports the engineering contract that neutral defaults preserve headline behavior.",
        "what_was_not_shown": "This gate does not evaluate non-default J-N4 dials.",
    }
